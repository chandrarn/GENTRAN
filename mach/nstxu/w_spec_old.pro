...
;6/28/16 - added shot specific change in coefs
PRO wspec_load_vuv,shot,specbr,lam,time,xeus=xeus,loweus=loweus,monalisa=monalisa,status=status	
	IF keyword_set(xeus) THEN BEGIN
		path='\PASSIVESPEC::TOP.XEUS'
		;coefs=[63.87957,-0.05163,3.3643e-6]
		coefs=[9.60775189,0.02726005,0.00001421]
		IF shot LT 204937 THEN coefs=[65.6602054,-0.063991,1.57e-5]
		npix=1340
	ENDIF
	IF keyword_set(loweus) THEN BEGIN
		path='\PASSIVESPEC::TOP.LOWEUS'	
		coefs=[193.241879,0.15330535,0.00002196]
		IF shot LT 204937 THEN coefs=[440.57775,-0.21256,2.07675e-5]
		npix=1340
	ENDIF
	IF keyword_set(monalisa) THEN BEGIN
		path='\PASSIVESPEC::TOP.MONALISA'	
		coefs=[37.9139431,0.08950414,0.00001878]
		IF shot LT 204937 THEN coefs=[224.19661,-0.15881,2.28886e-5]
		npix=1340
	ENDIF
	mdsopen,'passivespec',shot
	specbr=float((mdsvalue('_sig='+path+'.RAWDATA:SPECTRA',/quiet,status=status)))
	;lam=mdsvalue('dim_of(_sig,0)',/quiet)
	x=findgen(npix)
	lam=coefs[0]+x*coefs[1]+x^2*coefs[2]
	time=float(mdsvalue(path+'.DETECTOR:TIMES',/quiet))
	;time=float(mdsvalue('dim_of(_sig,2)',/quiet))/1.0e3
...
PRO wspec_load_monalisa,shot,specbr,lam,time,status=status
	wspec_load_vuv,shot,ispecbr,lam,time,/mona,status=status
;	IF NOT status THEN BEGIN
;		load_xeus,shot,cnts
;		nframes=n(cnts[0,*])+1
;		xeus_lam_time,lam
...
PRO wspec_load_dalpha,shot,time,dalpha,status=status
	mdsopen,'passivespec',shot
	dalpha=mdsvalue('_sig=\DA_LD',/quiet,status=status)
...
PRO wspec_load_dens,shot,time,dens,status=status
	...
	dens=mdsvalue('_sig=\ts_ld',/quiet,status=status)
    	time=mdsvalue('dim_of(_sig,0)',/quiet)
	mdsclose,'activespec',shot
	dens*=1.0e-14/130.0	;put in 10^20/m^3
END
...
PRO wspec_calc_br,u,line
	...
	spec='none'
	IF lamc GT 5 AND lamc LT 70 THEN spec='xeus'
	IF lamc GT 70 AND lamc LT 200 THEN spec='mona'
	IF lamc GT 200 AND lamc LT 450 THEN BEGIN
		IF u.stat.low THEN spec='loweus'
	ENDIF
	IF lamc GT 450 THEN spec='spredl'

	IF spec EQ 'xeus' AND u.stat.xeus NE 1 THEN spec='x'
	IF spec EQ 'mona' AND u.stat.mona NE 1 THEN spec='x'
	...
	CASE spec OF 
		'xeus' : BEGIN
			...
			*u.dat.br[line]=br
                END
		'mona' : BEGIN
			ntime=n(u.dat.mona.time)+1
			*u.dat.br[4+line]=u.dat.mona.time
			tmp=where(u.dat.mona.lam GE u.line.lam0[line] AND u.dat.mona.lam LE u.line.lam1[line])
			br=sum_array(u.dat.mona.specbr[tmp,*],/j)-0.5*(u.dat.mona.specbr[tmp[0],*]+u.dat.mona.specbr[last(tmp),*])*(n(tmp)+1)
			*u.dat.br[line]=br
                END
		'loweus' : BEGIN
...
PRO wspec_plot_time,u
	...
	IF size(u.dat.dens,/type) EQ 8 THEN BEGIN
		dtmp=where(tau GE u.dat.dens.time[0] AND tau LE last(u.dat.dens.time))
		IF dtmp[0] NE -1 THEN oplot,tau[dtmp],interpol(u.dat.dens.dens,u.dat.dens.time,tau[dtmp])
	ENDIF
	IF u.stat.tline THEN oplot,tau0,yr,linestyle=1.0,color=200
...
FUNCTION wspec_maxsbr,u
	...
	IF u.stat.low AND u.plot.low THEN BEGIN
        ...
        ENDIF
	IF u.stat.mona AND u.plot.mona THEN BEGIN
		index=ipt(u.stat.time,u.dat.mona.time)	
		tmp=where(u.dat.mona.lam GE xr[0] AND u.dat.mona.lam LE xr[1])
		IF tmp[0] NE -1 THEN ymax=ymax > max(u.dat.mona.specbr[tmp,index])
        ENDIF
...
PRO wspec_plot_spec,u
	...
	IF u.stat.mona AND u.plot.mona THEN BEGIN
		index=ipt(u.stat.time,u.dat.mona.time)
		IF index NE -1 THEN oplot,u.dat.mona.lam,u.dat.mona.specbr[*,index],col=col[0]
		u.dat.mona.index=index
        ENDIF 
			...
			CASE u.line.spec[i] OF
				'xeus' : BEGIN
					...
					ENDIF
	                        END
				'mona' : BEGIN
					IF u.stat.mona THEN BEGIN
						tmp=where(u.dat.mona.lam GE u.line.lam0[i] AND u.dat.mona.lam LE u.line.lam1[i])
						index=ipt(u.stat.time,u.dat.mona.time)
						IF index NE -1 THEN oplot,u.dat.mona.lam[tmp],u.dat.mona.specbr[tmp,index],col=lcol[i]
					ENDIF
                                END
...
PRO wspec_load_data,u
	...
	;load MONA LISA spectra
	IF u.stat.load.m THEN wspec_load_monalisa,shot,specbr,lam,time,status=status ELSE status=0
	IF status THEN BEGIN
		mona={specbr:specbr,lam:lam,time:time,index:ipt(time,u.stat.time)} 
		u.stat.mona=1
		if max(time) gt u.stat.tmax then u.stat.tmax=max(time)
		IF min(lam) LT u.stat.lmin THEN u.stat.lmin=min(lam)
		IF max(lam) GT u.stat.lmax THEN u.stat.lmax=max(lam)
		widget_control,u.id.message,set_value='SHOT '+num2str(shot,1)+': MONA LISA Loaded',/append
		widget_control,u.id.mona,set_button=1
        ENDIF ELSE BEGIN
		mona=-1
		u.stat.mona=0
	...
	u.stat.dat=1 
	dat={low:low,mona:mona,xeus:xeus,spredl:spredl,spreds:spreds,dalpha:dalpha,xtomo:xtomo,dens:dens,nbi:nbi,rf:rf,line:line,br:br,sbr:sbr}
	...
	CASE tag OF
		...
		"WIDGET_BUTTON": BEGIN
			...
			CASE ename OF
				...
				"MONA" : BEGIN
					IF u.stat.dat THEN BEGIN
						IF event.select EQ 1 AND u.stat.mona THEN u.plot.mona=1 ELSE u.plot.mona=0
						wspec_plot_spec,u
                                        ENDIF
                                END
				"LOW" : BEGIN
				...
				'LMONA' : IF event.select EQ 1 THEN u.stat.load.m=1 ELSE u.stat.load.m=0
...
PRO w_spec,shot=shot,time=time
	IF NOT keyword_set(shot) THEN shot=204577
	...
	lmona=widget_button(C1p1y,value=' M ')
	...
	mona=widget_button(C1p3,value=' MONA LISA ')
	low=widget_button(C1p3,value=' LoWEUS ')
	C1p4=widget_base(C1,/row,/nonexcl)
	spreds=widget_button(C1p4, value=' SPRED (s)')
	spredl=widget_button(C1p4, value=' SPRED (l)')
	...
	id={base:base,draw1:draw1,draw2:draw2,t_slider:t_slider,t_text:t_text,$
		...
		lspred:lspred,llow:llow,lxeus:lxeus,lmona:lmona,$
		xeus:xeus,low:low,mona:mona,spreds:spreds,spredl:spredl,$
	...
	load={s:0,l:1,m:1,x:1}
	stat={time:time,tmax:2.0,lmin:0.0,lmax:0.0,dat:0,ps:0,col:!p.color,pscol:0,tline:1,load:load,xeus:0,mona:0,low:0,spreds:0,spredl:0}
	plot={	asize:[400,ysz],$
		bsize:[ysz,ysz],$
		...
		x1:[2.0,450.0],$
		zoom:[0,0],$
		xeus:1,	mona:1, low:1,	spreds:0, spredl:0,$
	...
	line={	label:['C VI',	'N VII',	'O VIII',	'Fe XXIII'],$
		...
		lam1: [34.07,		25.08,		19.08,		134.5],$
		plot:[1,1,1,1],$
		col:[50,90,130,180],$
		pscol:[30,100,150,200],$
		spec:['xeus','xeus','xeus','mona']$
		}
	...
	widget_control, u.id.lmona,set_button=u.stat.load.m
